...
const VendorListPage = () => {
    ...
    getAllVendors('3fa85f64-5717-4562-b3fc-2c963f66afa6')
      .then((vendorList) => {
        if (Array.isArray(vendorList)) {
          const customersWithSequentialId = vendorList.map((vendor, index) => ({
            ...vendor,
  ...
  const renderRowSubComponent = useCallback(
    ({ row }: { row: Row<{}> }) => <VendorView data={memoizedCustomers[Number(row.id)]} />,
    [memoizedCustomers]
  );
  ...
  return (
    <MainCard content={false}>
      ...
      >
        <AddVendor vendor={customer} onCancel={handleAdd} />
      </Dialog>
    </MainCard>
  );
};
